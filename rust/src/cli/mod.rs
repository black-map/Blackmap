...
impl From<ScanTypeArg> for crate::config::ScanType {
    fn from(arg: ScanTypeArg) -> Self {
        match arg {
            ScanTypeArg::TcpConnect => crate::config::ScanType::TcpConnect,
            ScanTypeArg::TcpSyn => crate::config::ScanType::TcpSyn,
            ScanTypeArg::Udp => crate::config::ScanType::Udp,
            ScanTypeArg::IcmpPing => crate::config::ScanType::IcmpPing,
        }
    }
}
...
pub fn parse_ports(spec: &str) -> crate::error::Result<Vec<u16>> {
    ...
    for part in spec.split(',') {
        ...
        if part.contains('-') {
            ...
            if range_parts.len() != 2 {
                return Err(crate::error::BlackMapError::ConfigError(
                    format!("Invalid port range: {}", part),
                ));
            }

            let start: u16 = range_parts[0].parse()
                .map_err(|_| crate::error::BlackMapError::ConfigError(
